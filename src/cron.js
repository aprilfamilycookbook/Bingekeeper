...
async function getUpcomingNotifications(env) {
  const baseQuery = `SELECT w.user_id, w.show_id, u.email, u.name, s.name as show_name, s.next_episode_date, s.next_season_number, s.next_episode_number`;
  const joins = ` FROM watchlist w JOIN users u ON w.user_id = u.id JOIN shows s ON w.show_id = s.id WHERE w.notify = 1 AND u.notify_email = 1 AND s.next_episode_date IS NOT NULL AND s.next_episode_date BETWEEN date('now') AND date('now', '+2 days')`;
  try {
