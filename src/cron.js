...
async function checkShowForUpdates(showId, env) {
  try {
    const res = await fetch(`https://api.themoviedb.org/3/tv/${showId}?api_key=${env.TMBD_API_KEY}&language=en-US`);
...
async function sendNotification(env, item) {
  ...
  const subject = isNewSeason ? `èé¨ New season of ${item.show_name} starts soon!` : `üì∫ New episode of ${item.show_name} tomorrow!`;
