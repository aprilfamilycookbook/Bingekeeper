...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (params.get('token') && window.location.pathname === '/verify') {
    showVerify(params.get('token'));
  } else if (params.get('token') && window.location.pathname === '/reset') {
...
function showReset(t) {
  ...
  document.getElementById('resetForm').dataset.token = t;
}

function showVerify(t) {
  document.getElementById('authPage').classList.remove('hidden');
  hideAllAuthForms();
...
function hideAllAuthForms() {
  ['loginForm','registerForm','forgotForm','resetForm','verifyForm'].forEach(id => {
...
async function doReset() {
  ...
  if (res.error) { showError(errEl, res.error); return; }
  successEl.textContent = res.message + ' Redirecting...';
  successEl.classList.remove('hidden');
...
async function showApp() {
  ...
  document.addEventListener('click', e => {
    ...
    if (!sr.contains(e.target) && e.target !== document.getElementById('searchInput') && e.target !== document.getElementById('searchBtn')) {
...
async function doSearch() {
  ...
  if (res.error) {
    resultsEl.innerHTML = `<div class="search-msg error">${esc(res.error)}</div>`;
    return;
  }
  ...
  resultsEl.innerHTML = res.results.slice(0, 6).map(s => `
    <div class="result-item">
      ${s.poster_path
        ? `<img class="result-poster" src="https://image.tmdb.org/t/p/w92${s.poster_path}" alt="${esc(s.name)}" style="object-fit:cover">`
        : `<div class="result-poster">📺</div>`}
      <div class="result-info">
        <div class="result-name">${esc(s.name)}</div>
        <div class="result-year">${(s.first_air_date || '').slice(0,4) || 'Unknown'}</div>
        ${s.overview ? `<div class="result-overview">${esc(s.overview)}</div>` : ''}
      </div>
      <button class="btn-add" onclick='openAdd(${JSON.stringify(s)})'>+ Add</button>
    </div>`).join('');
}
...
function openAdd(show) {
  const alreadyAdded = watchlist.find(w => w.show_id === show.id);
  if (alreadyAdded) { toast('Already in your watchlist!'); return; }
  ...
  document.getElementById('modalBody').innerHTML = `
    ...
    <div class="form-row">
      <div class="form-group"><label>Season</label><input type="number" id="f-season" value="1" min="1" max="99"></div>
      <div class="form-group"><label>Episode</label><input type="number" id="f-episode" value="1" min="1" max="999"></div>
    </div>
    <div class="notify-row">
      <input type="checkbox" id="f-notify" checked>
      <label for="f-notify">📧 Email me when new episodes drop</label>
    </div>
    <div class="modal-actions">
      <button class="btn-cancel" onclick="closeModal()">Cancel</button>
      <button class="btn-save" onclick='confirmAdd(${JSON.stringify(show)})'>Add to watchlist</button>
    </div>`;
  openModal();
}
...
async function confirmAdd(show) {
  const body = {
    ...
    current_season: parseInt(document.getElementById('f-season').value) || 1,
    current_episode: parseInt(document.getElementById('f-episode').value) || 1,
...
function openEdit(idx) {
  ...
  document.getElementById('modalBody').innerHTML = `
    ...
    <div class="form-row">
      <div class="form-group"><label>Season</label><input type="number" id="f-season" value="${s.current_season}" min="1" max="99"></div>
      <div class="form-group"><label>Episode</label><input type="number" id="f-episode" value="${s.current_episode}" min="1" max="999"></div>
    </div>
    <div class="notify-row">
      <input type="checkbox" id="f-notify" ${s.notify ? 'checked' : ''}>
      <label for="f-notify">📧 Email me when new episodes drop</label>
    </div>
    <div class="modal-actions">
      <button class="btn-cancel" onclick="closeModal()">Cancel</button>
      <button class="btn-save" onclick="confirmEdit(${s.show_id})">Save changes</button>
    </div>`;
  openModal();
}
...
function render() {
  ...
  if (!list.length) {
    grid.innerHTML = `<div class="empty-state">
      ...
      <h3>${watchlist.length === 0 ? 'Your watchlist is empty' : 'Nothing here yet'}</h3>
      <p>${watchlist.length === 0 ? 'Search for a show above to get started!' : 'Add some shows to this category.'}</p>
    </div>`;
    return;
  }
  ...
  grid.innerHTML = list.map((s, i) => {
    ...
    const hasUpcoming = s.next_episode_date && s.next_episode_date >= new Date().toISOString().slice(0,10);
    const nextLabel = s.next_episode_date ? `S${s.next_season_number}E${s.next_episode_number} · ${s.next_episode_date}` : null;

    return `<div class="show-card">
      ${s.poster_path
        ? `<img class="show-poster" src="https://image.tmdb.org/t/p/w185${s.poster_path}" alt="${esc(s.name)}" loading="lazy">`
        : `<div class="show-poster-ph">📺</div>`}
      ${hasUpcoming ? `<div class="upcoming-badge">New Soon</div>` : ''}
      <div class="show-body">
        <div class="show-title" title="${esc(s.name)}">${esc(s.name)}</div>
        <div class="show-badges">
          <span class="badge ${STATUS_BADGE[s.status]||'b-watching'}">${s.status}</span>
          <span class="badge b-service">${esc(s.service)}</span>
        </div>
        <div class="show-progress">S${s.current_season} · E${s.current_episode}</div>
        ${nextLabel ? `<div class="show-next">📅 ${nextLabel}</div>` : ''}
        <div class="show-actions">
          <button class="btn-sm" onclick="openEdit(${idx})">✏ Edit</button>
          <button class="btn-sm btn-sm-danger" onclick="removeShow(${s.show_id}, '${esc(s.name)}')">🗑</button>
        </div>
      </div>
    </div>`;
  }).join('');
...
async function api(path, method = 'GET', body = null) {
  try {
    ...
    if (res.status === 401 && path !== '/api/auth/login') {
      doLogout(); return { error: 'Session expired' };
    }
