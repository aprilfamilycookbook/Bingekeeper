...
export async function handleAuth(request, env, path) {
  ...
  if (path === '/api/auth/register' && request.method === 'POST') {
    ...
    await env.DB.prepare('INSERT INTO users (email, password_hash, name, verify_token) VALUES (?, ?, ?, ?)')
      .bind(email.toLowerCase(), hash, name, token).run();

    // Send verification email
    const verifyUrl = `https://bingekeeper.tv/verify?token=${token}`;
    await sendEmail(env, email, 'Verify your Bingekeeper account', `
      <h2>Welcome to Bingekeeper, ${name}!</h2>
      <p>Click below to verify your email address:</p>
      <a href="${verifyUrl}" style="background:#378ADD;color:white;padding:12px 24px;border-radius:8px;text-decoration:none;display:inline-block">Verify Email</a>
  ...
  if (path === '/api/auth/forgot' && request.method === 'POST') {
    ...
    await sendEmail(env, email, 'Reset your Bingekeeper password', `
      <h2>Password Reset</h2>
      <p>Click below to reset your password. This link expires in 1 hour.</p>
      <a href="${resetUrl}" style="background:#378ADD;color:white;padding:12px 24px;border-radius:8px;text-decoration:none;display:inline-block">Reset Password</a>
    `);
...
async function sendEmail(env, to, subject, html) {
  ...
  await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: 'Bingekeeper <hello@bingekeeper.tv>',
      to,
      subject,
      html
    })
